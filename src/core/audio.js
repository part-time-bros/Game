...
export class AudioEngine {
  ...
  play(name, opts = {}) {
    ...
    switch (name) {
      case 'shoot': {
        ...
        this._tone({ type: 'sawtooth', f0: f, f1: f * 0.32, dur: 0.11, gain: 0.16 * v, filter: 'bandpass', cutoff0: f * 2.1, cutoff1: f * 0.8, q: 2.4 });
        this._noiseBurst({ f0: 3200, f1: 900, dur: 0.06, gain: 0.09 * v, q: 0.9 });
        break;
      }
      case 'shootHeavy': {
      ...
      case 'enemyShoot': {
        ...
        this._tone({ type: 'square', f0: 320 * p, f1: 140 * p, dur: 0.13, gain: 0.075 * v, filter: 'bandpass', cutoff0: 900, cutoff1: 380, q: 3 });
        break;
      }
      case 'hit': {
        if (!this._gate('hit', 0.022)) return;
        this._noiseBurst({ f0: 2600 * p, f1: 700, dur: 0.055, gain: 0.11 * v, q: 1.6 });
        this._tone({ type: 'triangle', f0: 560 * p, f1: 200, dur: 0.05, gain: 0.07 * v });
        break;
      }
      case 'crit': {
        if (!this._gate('crit', 0.03)) return;
        this._noiseBurst({ f0: 5200, f1: 1400, dur: 0.09, gain: 0.13 * v, q: 2 });
        this._tone({ type: 'triangle', f0: 1500, f1: 620, dur: 0.13, gain: 0.1 * v });
        this._tone({ type: 'triangle', f0: 2260, f1: 940, dur: 0.1, gain: 0.06 * v, detune: 9 });
        break;
      }
      case 'kill': {
      ...
      case 'dash': {
        this._noiseBurst({ f0: 400, f1: 4200, dur: 0.17, gain: 0.16 * v, q: 1.4 });
        this._tone({ type: 'sine', f0: 420, f1: 90, dur: 0.24, gain: 0.13 * v });
        break;
      }
      case 'pulse': {
      ...
      case 'shieldUp':
        this._tone({ type: 'triangle', f0: 320, f1: 1400, dur: 0.35, gain: 0.09 * v, filter: 'bandpass', cutoff0: 700, cutoff1: 2600, q: 3 });
        break;
      case 'pickup': {
      ...
      case 'uiHover':
        ...
        this._tone({ type: 'square', f0: 1500, f1: 1800, dur: 0.035, gain: 0.03 * v, filter: 'bandpass', cutoff0: 2000, q: 4 });
        break;
      case 'uiClick':
        this._tone({ type: 'square', f0: 700, f1: 1400, dur: 0.06, gain: 0.06 * v, filter: 'bandpass', cutoff0: 1800, q: 3 });
        this._noiseBurst({ f0: 3000, f1: 6000, dur: 0.05, gain: 0.045 * v });
        break;
      case 'uiBack':
        this._tone({ type: 'square', f0: 900, f1: 420, dur: 0.08, gain: 0.05 * v, filter: 'bandpass', cutoff0: 1400, q: 3 });
        break;
      case 'uiDeny':
      ...
      case 'charge': {
        ...
        this._tone({ type: 'sawtooth', f0: 180, f1: 1500, dur: opts.dur || 0.9, gain: 0.075 * v, filter: 'bandpass', cutoff0: 400, cutoff1: 2400, q: 6 });
        break;
      }
      case 'beam':
  ...
  _scheduler() {
    if (!this.ready || !this.musicOn || this.ctx.state !== 'running') return;
